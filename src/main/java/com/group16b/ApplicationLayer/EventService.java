...
public class EventService {
	...
	@Value("${virtual-queue.pass-num:50}")
	private int virtualQueuePassNum = 50;
	...
	public Result<EventDTO> createEvent(EventRecord eventRecord, String sessionToken) {
		try {
			...
			logger.error(
					"EventService.createEvent: Concurrent modification error during event creation: " + e.getMessage());
		...
		return params.get(key).stream().map(val -> {
			if (!type.isInstance(val)) {
				throw new IllegalArgumentException(
						"Invalid type for parameter '" + key + "'. Expected: " + type.getSimpleName());
			}
			return (T) val;
		}).toList();
	}
