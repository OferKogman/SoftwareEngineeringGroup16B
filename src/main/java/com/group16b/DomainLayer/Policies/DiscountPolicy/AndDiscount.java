...
public class AndDiscount implements DiscountPolicy {
    private final List<DiscountPolicy> children;
    private final double discountPercentage;

    public AndDiscount(List<DiscountPolicy> children, double discountPercentage) {
        if (children == null || children.isEmpty())
            throw new IllegalArgumentException("AndDiscount must have at least one child.");
    ...
    public boolean isMet(DiscountContext context) {
        return children.stream().allMatch(child -> child.isMet(context));
    }
