...
public class LotteryPolicyController extends BaseController {
    ...
    public ResponseEntity<?> createLotteryPolicy(
        ...
        return executeWithNoReturnData(() -> lotteryPolicyService.createLotteryPolicy(eventId, request.lotteryID(),
                request.lotteryName(), request.winnerAmount(), request.lotteryRegistrationDueDate(), authToken));
    }
    ...
    public ResponseEntity<?> enrollInLottery(
            @PathVariable("eventId") int eventId,
            @RequestHeader("Authorization") String authToken) {
        return executeWithNoReturnData(() -> lotteryPolicyService.enrollInLottery(eventId, authToken));
    }
    ...
    public ResponseEntity<?> handleLotteryResults(
            @PathVariable("eventId") int eventId,
            @RequestHeader("Authorization") String authToken) {
        return executeWithNoReturnData(() -> lotteryPolicyService.handleLotteryResults(eventId, authToken));
    }
